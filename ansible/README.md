...
# edit group_vars/all/all.yml: stuudio_domain, stuudio_release_version if you want to pin
...
Fully idempotent - installs libvips42 + Caddy, creates the `stuudio` system user and
`/opt/stuudio/data`, templates the env file and systemd unit from your vault values, fetches the
...
Fetches whatever `stuudio_release_version` resolves to (default `"latest"`, tracking the most
...
```bash
ansible-playbook update.yml --ask-vault-pass -e stuudio_release_version=v1.2.3
```

(or set `stuudio_release_version: "v1.2.3"` in `group_vars/all/all.yml` to make the pin permanent).

## What's not automated

- **DNS** - point `stuudio_domain`'s `A`/`AAAA` record at your VPS yourself before the Caddy step
  runs, or Let's Encrypt's HTTP challenge will fail.
- **Backups** - see `../docs/deploy.md`'s Backups section; `/opt/stuudio/data/` is the whole
